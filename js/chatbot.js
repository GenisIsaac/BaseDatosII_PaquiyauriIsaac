// =============================================
// BD2 BOT - Chatbot con Hugging Face API (via Supabase Edge Function)
// =============================================
...
async function queryHF(message) {
  ...
  const functionUrl = cfg.url.replace(/\/$/, '') + '/functions/v1/chat';
...
// ─── DB Thought Bubbles ─────────────────────────
const DB_PREFACES = ['¿Sabías que…', 'Dato curioso:', 'Dato freak:', 'Fun fact:', '¿Lo sabías?', 'Curiosidad BD:'];
const DB_FACTS = [
  'el modelo relacional fue propuesto por Edgar Codd en 1970 🧠',
  'SQL originalmente se llamaba SEQUEL, creado por IBM en los 70s',
  'la BD más grande del mundo es del CERN: 200+ petabytes 🚀',
  'SQLite está en TODOS los smartphones del mundo 📱',
  'MySQL fue creado por Michael Widenius en 1995',
  'PostgreSQL nació en 1996 como sucesor de POSTGRES',
  'Redis puede manejar 1M+ operaciones por segundo ⚡',
  'MongoDB significa "humongous" (enorme)',
  'Oracle fue la primera compañía en comercializar SQL (1979)',
  'NoSQL significa "Not Only SQL", no "No SQL"',
  'el primer DBMS fue el Integrated Data Store (IDS) en los 60s',
  'GraphQL fue creado por Facebook en 2012',
  'Amazon DynamoDB escala a 20M+ requests/segundo',
  'el término "base de datos" se acuñó en 1963 📜',
  'las window functions existen en SQL desde SQL:2003',
  'el primer bug de software se encontró en el Harvard Mark II (1947)',
];
...
export function initThoughts() {
  const el = document.getElementById('chatThought');
  if (!el) return;

  function showNext() {
    const fact = DB_FACTS[rand(0, DB_FACTS.length - 1)];
    const preface = DB_PREFACES[rand(0, DB_PREFACES.length - 1)];
    el.textContent = preface + ' ' + fact;
    el.classList.add('show');
    clearTimeout(thoughtTimer);
    thoughtTimer = setTimeout(() => {
      el.classList.remove('show');
      thoughtTimer = setTimeout(showNext, rand(12000, 25000));
    }, rand(6000, 10000));
  }

  thoughtTimer = setTimeout(showNext, rand(4000, 8000));
}
