...
let state = null;
let currentUnit = 0;
window._editUnitIdx = 0;
window._editWeekIdx = 0;
...
function loadLocalData(){
  ...
  d.unidades.forEach(u=>u.semanas.forEach(s=>{
    s.archivos=s.archivos||[];
    s.podcast=s.podcast||null;
    if(!s.temas){
      const txt = s.desc || s.descripcion || '';
      s.temas = txt.split(/[,.;]+/).map(t=>t.trim()).filter(t=>t.length>2);
    }
  }));
  return d;
}
...
function buildWeekModalContent(unitIdx, weekIdx){
  ...
  const topicPoints = (s.temas && s.temas.length) ? s.temas.filter(t=>t.trim().length>0) : descText.split(/[,.;]+/).filter(t => t.trim().length > 4).map(t => t.trim());
...
function openAdmin(){
  ...
  const tubesCanvas = document.getElementById('tubesCanvas');
...
window.logout = function(){
  ...
  const tubesCanvas = document.getElementById('tubesCanvas');
  const auroraCanvas = document.getElementById('auroraCanvas');
  if(tubesCanvas) tubesCanvas.style.display = '';
...
function renderAdminContent(page){
  ...
  try {
  if(page==='content'){
    const selUnit = typeof window._editUnitIdx === 'number' ? window._editUnitIdx : 0;
    const selWeek = typeof window._editWeekIdx === 'number' ? window._editWeekIdx : 0;
    const unit = state.unidades[selUnit];
    const week = unit ? unit.semanas[selWeek] : null;
    const weekFileCount = week ? (week.archivos||[]).length : 0;
    const hasPodcast = week && week.podcast;

    c.innerHTML = `
      <div class="admin-page-title"><i class="fas fa-pen-to-square" style="margin-right:10px"></i>Editar Contenido</div>
      <div class="admin-page-sub">Selecciona una unidad y semana para editar su información.</div>

      <div class="edit-filter-bar">
        <div class="edit-filter-group">
          <label><i class="fas fa-layer-group"></i> Unidad</label>
          <select id="editUnitSelect" onchange="window._onEditUnitChange(this.value)">
            ${state.unidades.map((u,i)=>`<option value="${i}" ${i===selUnit?'selected':''}>U${i+1} — ${u.nombre}</option>`).join('')}
          </select>
        </div>
        <div class="edit-filter-group">
          <label><i class="fas fa-calendar-week"></i> Semana</label>
          <select id="editWeekSelect" onchange="window._onEditWeekChange(this.value)">
            ${unit ? unit.semanas.map((s,i)=>`<option value="${i}" ${i===selWeek?'selected':''}>S${i+1} — ${s.nombre}</option>`).join('') : '<option>Sin semanas</option>'}
          </select>
        </div>
        ...
        <div class="edit-col-main">
          ...
          <div class="edit-section-card">
            <div class="edit-section-head">
              <div class="edit-section-icon unit"><i class="fas fa-layer-group"></i></div>
              <div>
                <div class="edit-section-label">Unidad ${selUnit+1}</div>
                <div class="edit-section-title">${unit ? unit.nombre : '—'}</div>
              </div>
              <button class="fdel" data-hot onclick="deleteUnit(${selUnit})" title="Eliminar unidad"><i class="fas fa-trash"></i></button>
            </div>
            <div class="edit-section-body">
              <div class="edit-field">
                <label>Nombre</label>
                <input class="editable-name" value="${unit ? unit.nombre : ''}" onchange="updateUnit(${selUnit},'nombre',this.value)">
              </div>
              <div class="edit-field">
                <label>Descripción</label>
                <textarea class="desc-edit" rows="2" onchange="updateUnit(${selUnit},'descripcion',this.value)">${unit ? unit.descripcion : ''}</textarea>
              </div>
            </div>
          </div>
          ...
          <div class="edit-section-card">
            <div class="edit-section-head">
              <div class="edit-section-icon week"><i class="fas fa-calendar-week"></i></div>
              <div>
                <div class="edit-section-label">Semana ${selWeek+1} de Unidad ${selUnit+1}</div>
                <div class="edit-section-title">${week ? week.nombre : '—'}</div>
              </div>
              ${week ? `<button class="fdel" data-hot onclick="deleteWeek(${selUnit},${selWeek})" title="Eliminar semana"><i class="fas fa-trash"></i></button>` : ''}
            </div>
            <div class="edit-section-body">
              ${week ? `
              <div class="edit-field">
                <label>Nombre</label>
                <input class="aweek-name" value="${week.nombre}" onchange="updateWeek(${selUnit},${selWeek},'nombre',this.value)">
              </div>
              <div class="edit-field">
                <label>Descripción</label>
                <textarea class="aweek-desc" rows="3" placeholder="Descripción de la semana" onchange="updateWeek(${selUnit},${selWeek},'descripcion',this.value)">${week.descripcion||week.desc||''}</textarea>
              </div>
              <div class="edit-temas-section">
                <div class="edit-temas-head">
                  <div class="edit-temas-label"><i class="fas fa-list-check"></i> Temas de la semana</div>
                  <span class="edit-temas-count">${(week.temas||[]).length}</span>
                </div>
                <div class="edit-temas-list" id="editTemaList">
                  ${(week.temas||[]).map((t,ti)=>`
                    <div class="edit-tema-item" data-idx="${ti}">
                      <span class="edit-tema-num">${ti+1}</span>
                      <input class="edit-tema-input" value="${t.replace(/"/g,'&quot;')}" onchange="updateTema(${selUnit},${selWeek},${ti},this.value)" placeholder="Tema...">
                      <button class="edit-tema-move" onclick="moveTema(${selUnit},${selWeek},${ti},-1)" title="Subir" ${ti===0?'disabled':''}><i class="fas fa-chevron-up"></i></button>
                      <button class="edit-tema-move" onclick="moveTema(${selUnit},${selWeek},${ti},1)" title="Bajar" ${ti===(week.temas||[]).length-1?'disabled':''}><i class="fas fa-chevron-down"></i></button>
                      <button class="edit-tema-del" onclick="removeTema(${selUnit},${selWeek},${ti})" title="Eliminar"><i class="fas fa-xmark"></i></button>
                    </div>
                  `).join('')}
                </div>
                <button class="add-tema-btn" onclick="addTema(${selUnit},${selWeek})"><i class="fas fa-plus"></i> Añadir tema</button>
              </div>
              ` : `
              <div class="edit-empty-inline">
                <i class="fas fa-folder-open"></i>
                <p>No hay semanas en esta unidad</p>
              </div>
              `}
            </div>
          </div>
          ...
          <div class="edit-actions-row">
            <button class="add-week-btn" data-hot onclick="addWeek(${selUnit})"><i class="fas fa-plus"></i> Añadir Semana</button>
            <button class="btn-save" data-hot onclick="saveAll()"><i class="fas fa-floppy-disk"></i> Guardar Cambios</button>
          </div>

        </div>

        <div class="edit-col-side">
          <div class="edit-side-header">Todas las Unidades</div>
          <div class="edit-side-list">
            ${state.unidades.map((u,ui)=>`
              <div class="edit-side-unit ${ui===selUnit?'active':''}" onclick="window._editUnitIdx=${ui};window._editWeekIdx=0;renderAdminContent('content')">
                <div class="esu-head">
                  <div class="esu-num">U${ui+1}</div>
                  <div class="esu-info">
                    <div class="esu-name">${u.nombre}</div>
                    <div class="esu-count">${u.semanas.length} sem · ${u.semanas.reduce((a,s)=>a+(s.archivos||[]).length,0)} arch</div>
                  </div>
                </div>
                <div class="esu-weeks">
                  ${u.semanas.map((s,si)=>{
                    const fCount = (s.archivos||[]).length;
                    const isSel = ui===selUnit && si===selWeek;
                    return `<div class="esu-week ${isSel?'active':''}" onclick="event.stopPropagation();window._editUnitIdx=${ui};window._editWeekIdx=${si};renderAdminContent('content')">
                      <span class="esw-num">S${si+1}</span>
                      <span class="esw-name">${s.nombre}</span>
                      ${fCount > 0 ? `<span class="esw-count">${fCount}</span>` : ''}
                    </div>`;
                  }).join('')}
                </div>
              </div>
    ...
    let totalFiles = 0, totalPodcasts = 0, totalWeeks = 0;
    const typeCount = {};
    const recentFiles = [];
    const now = Date.now();
      ...
      u.semanas.forEach(s => {
        ...
        (s.archivos || []).forEach(a => {
          ...
          if (age !== null && age < 10080) {
            recentFiles.push({...a, unit: u.nombre, week: s.nombre, weekId: s.id});
          }
        });
    ...
    const maxSize = topTypes.length ? topTypes[0][1] : 1;
      ...
      <div class="dash-wrapper">
        <div class="dash-header">
          <div>
            ...
            <p class="dash-subtitle">Resumen del repositorio académico — Base de Datos II</p>
          </div>
          <div class="dash-header-actions">
            <button class="dash-icon-btn" onclick="showToast('Repositorio actualizado')">
        ...
        <div class="dash-stats-grid">
          <div class="dash-stat-card">
            ...
            <div class="dash-stat-label">Unidades</div>
            <div class="dash-stat-value">${state.unidades.length}</div>
            <div class="dash-stat-change up">Módulos del curso</div>
          </div>
          <div class="dash-stat-card">
            <div class="dash-stat-header">
              <div class="dash-stat-icon green"><i class="fas fa-calendar-week"></i></div>
            </div>
            <div class="dash-stat-label">Semanas</div>
            <div class="dash-stat-value">${totalWeeks}</div>
            <div class="dash-stat-change up">${state.unidades.length > 0 ? Math.round(totalWeeks / state.unidades.length) : 0} promedio por unidad</div>
          </div>
          <div class="dash-stat-card">
            <div class="dash-stat-header">
              <div class="dash-stat-icon purple"><i class="fas fa-file"></i></div>
            </div>
            <div class="dash-stat-label">Archivos</div>
            <div class="dash-stat-value">${totalFiles}</div>
            <div class="dash-stat-change up">${Object.keys(typeCount).length} tipos distintos</div>
          </div>
          <div class="dash-stat-card">
            <div class="dash-stat-header">
              <div class="dash-stat-icon orange"><i class="fas fa-podcast"></i></div>
            </div>
            <div class="dash-stat-label">Podcasts</div>
            <div class="dash-stat-value">${totalPodcasts}</div>
            <div class="dash-stat-change up">${totalWeeks - totalPodcasts} semanas sin podcast</div>
          </div>
        </div>
        ...
        <div class="dash-content-grid">
          <div class="dash-card-wide">
            <div class="dash-card-header">
              ...
              <button class="dash-card-link" onclick="showAdminPage('files')">Ver todo</button>
            </div>
            <div class="dash-activity-list">
              ${recentFiles.length ? recentFiles.slice(0, 6).map(a => {
                const ft = getFileType(a.nombre);
                ...
                <div class="dash-activity-empty">
                  ...
                  <p>Sin actividad reciente — sube archivos para comenzar</p>
                </div>
              `}
            </div>
          </div>
          ...
          <div class="dash-card-narrow">
            ...
            <div class="dash-quick-stats">
              ${topTypes.length ? topTypes.map(([ext, count], tIdx) => {
                const pct = Math.round((count / maxSize) * 100);
                const colorClasses = ['blue','orange','green','purple','red'];
                const cc = colorClasses[tIdx % colorClasses.length];
                return `
                  <div class="dash-qs-row">
                    <span class="dash-qs-label">.${ext}</span>
                    <span class="dash-qs-value">${count} archivo(s)</span>
                  </div>
                  <div class="dash-qs-bar"><div class="dash-qs-fill ${cc}" style="width:${pct}%"></div></div>
                `;
              }).join('') : `
                <div style="text-align:center;padding:20px;color:var(--muted);font-size:13px">
                  <i class="fas fa-folder-open" style="font-size:24px;margin-bottom:8px;display:block;opacity:0.4"></i>
            ...
            <div class="dash-top-products">
              ${state.unidades.map((u, i) => {
                const fileCount = u.semanas.reduce((a, s) => a + (s.archivos || []).length, 0);
                return `<div class="dash-tp-row">
                  <span>U${i+1} — ${u.nombre}</span>
                  <span class="dash-tp-price">${u.semanas.length} sem · ${fileCount} arch</span>
                </div>`;
              }).join('')}
            </div>
          </div>
...
window._onEditUnitChange = function(val){
  window._editUnitIdx = parseInt(val);
  window._editWeekIdx = 0;
  renderAdminContent('content');
};
window._onEditWeekChange = function(val){
  window._editWeekIdx = parseInt(val);
  renderAdminContent('content');
};
...
window.addTema = function(ui, si){
  const temas = state.unidades[ui].semanas[si].temas || [];
  temas.push('');
  state.unidades[ui].semanas[si].temas = temas;
  saveData();
  renderAdminContent('content');
  setTimeout(()=>{
    const inputs = document.querySelectorAll('.edit-tema-input');
    if(inputs.length) inputs[inputs.length-1].focus();
  },50);
};
window.removeTema = function(ui, si, ti){
  const temas = state.unidades[ui].semanas[si].temas || [];
  temas.splice(ti, 1);
  state.unidades[ui].semanas[si].temas = temas;
  saveData();
...
window.updateTema = function(ui, si, ti, val){
  const temas = state.unidades[ui].semanas[si].temas || [];
  temas[ti] = val;
  state.unidades[ui].semanas[si].temas = temas;
  saveData();
...
window.moveTema = function(ui, si, ti, dir){
  const temas = state.unidades[ui].semanas[si].temas || [];
  const ni = ti + dir;
  if(ni < 0 || ni >= temas.length) return;
  [temas[ti], temas[ni]] = [temas[ni], temas[ti]];
  state.unidades[ui].semanas[si].temas = temas;
  saveData();
  renderAdminContent('content');
  renderUnit();
...
window.addUnit = function(){
  ...
  state.unidades.push({
    ...
    semanas: [
      {id: id + 's1', nombre: 'Semana 1', desc: '', temas: []}
    ]
  });
  window._editUnitIdx = state.unidades.length - 1;
  window._editWeekIdx = 0;
  saveData();
  renderAdminContent('content');
  renderUnit();
...
window.deleteUnit = function(ui){
  ...
  if(window._editUnitIdx >= state.unidades.length) window._editUnitIdx = Math.max(0, state.unidades.length - 1);
  window._editWeekIdx = 0;
  saveData();
  renderAdminContent('content');
  renderUnit();
...
window.addWeek = function(ui){
  ...
  const id = u.id + 's' + n;
  u.semanas.push({id, nombre: 'Semana ' + n, desc: '', temas: []});
  window._editUnitIdx = ui;
  window._editWeekIdx = u.semanas.length - 1;
  saveData();
  renderAdminContent('content');
  renderUnit();
...
window.deleteWeek = function(ui, si){
  ...
  if(window._editWeekIdx >= state.unidades[ui].semanas.length) window._editWeekIdx = Math.max(0, state.unidades[ui].semanas.length - 1);
  window._editUnitIdx = ui;
  saveData();
  renderAdminContent('content');
  renderUnit();
