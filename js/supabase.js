...
export async function syncState(state) {
  ...
  try {
    const currentUnitIds = (state.unidades || []).map(u => u.id);
    const currentSemanaIds = (state.unidades || []).flatMap(u => (u.semanas || []).map(s => s.id));
    const currentArchivoIds = (state.unidades || []).flatMap(u => (u.semanas || []).flatMap(s => (s.archivos || []).map(a => a.id)));

    if (currentArchivoIds.length > 0) {
      await supabaseClient.from('archivos').delete().not('id', 'in', `(${currentArchivoIds.map(id => `"${id}"`).join(',')})`);
    }
    await supabaseClient.from('podcasts').delete().not('semana_id', 'in', `(${currentSemanaIds.map(id => `"${id}"`).join(',')})`);
    if (currentSemanaIds.length > 0) {
      await supabaseClient.from('semanas').delete().not('id', 'in', `(${currentSemanaIds.map(id => `"${id}"`).join(',')})`);
    }
    if (currentUnitIds.length > 0) {
      await supabaseClient.from('unidades').delete().not('id', 'in', `(${currentUnitIds.map(id => `"${id}"`).join(',')})`);
    }
